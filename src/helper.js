...
const getPosition = (gameBoard, moveChecks) => {
    for (let check = 0; check < moveChecks.length; check++) {
        for (let i = 0; i < moveChecks[check].max; i += moveChecks[check].step) {
            ...
            let series = "";
            if (gameBoard[i + moveChecks[check].indexes[0]] !== undefined &&
                gameBoard[i + moveChecks[check].indexes[1]] !== undefined &&
                gameBoard[i + moveChecks[check].indexes[2]] !== undefined &&
                gameBoard[i + moveChecks[check].indexes[3]] !== undefined) {
                series = gameBoard[i + moveChecks[check].indexes[0]].toString() +
                    gameBoard[i + moveChecks[check].indexes[1]].toString() +
                    gameBoard[i + moveChecks[check].indexes[2]].toString() +
                    gameBoard[i + moveChecks[check].indexes[3]].toString();
            }
    ...
    let moveChecks = [
        {
            ...
            indexes: [0, 4, 8, 12],
            max: 4,//Number of times we may have to loop
            step: 1//increase it by how many times
        },
        ...
        {
            indexes: [0, 5, 10, 15],
            step: 16,//as there is only one diagonal
            max: 16
        },
        //other diagonal
        {
